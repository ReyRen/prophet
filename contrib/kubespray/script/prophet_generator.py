...
def get_prophet_daemon_resource_request(cfg):
    ...
    if "qos-switch" not in cfg or cfg["qos-switch"] == "false":
        logger.info("Ignore calculate prophet daemon resource usage since qos-switch set to false")
        return ret

    prophet_daemon_services = ["node-exporter", "job-exporter", "log-manager"]
    prophet_source_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "../../../src")
    ...
    for prophet_daemon in prophet_daemon_services:
        deploy_template_path = os.path.join(prophet_source_path, "{0}/deploy/{0}.yaml.template".format(prophet_daemon))
        if os.path.exists(deploy_template_path):
            template = read_template(deploy_template_path)
            match = regex.search(template)
            if not match:
                logger.warning("Could not find resource request for service %s", prophet_daemon)
                continue
            ...
            logger.warning("Could not find resource request for PAI daemon %s", prophet_daemon)
    return ret


def get_min_free_resource(workers, node_resource_dict, prophet_daemon_resource_dict):
    """
    ...
    for node_name in workers:
        ...
        min_cpu = min(min_cpu, node_resource_dict["free"][node_name]["cpu-resource"] - \
            prophet_daemon_resource_dict["cpu-resource"] - reserved_cpu)
        min_mem = min(min_mem, node_resource_dict["free"][node_name]["mem-resource"] - \
            prophet_daemon_resource_dict["mem-resource"] - reserved_mem)
...
def get_hived_config(layout, cluster_config):
    ...
    for machine in layout['machine-list']:
        if 'prophet-worker' in machine and machine['prophet-worker'] == 'true':
    ...
    prophet_daemon_resource_dict = get_prophet_daemon_resource_request(cluster_config)

    for sku_name in skus:
        sku_mem_free, sku_cpu_free = get_min_free_resource(skus[sku_name]['workers'], node_resource_dict, prophet_daemon_resource_dict)
